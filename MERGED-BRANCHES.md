...
- **fix/input-enter-keybindings** updated on origin after integration: commit `53ed71c2a7` now always consumes `return` when prompt autocomplete selects, so `ctrl+enter` accepts slash-command completion instead of immediately submitting. Re-merged into this integration branch in commit `fa8af72f3c`.
- **feat/clickable-sidebar-mcps** updated on origin after integration: commit `490b4f787e` added a plugin-side MCP refresh hook so sidebar MCP toggles refresh host MCP state after connect/disconnect instead of snapping back to stale disabled status. Re-merged into this integration branch in commit `54b491ca55`.
