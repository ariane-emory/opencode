...
export function resolveTheme(theme: ThemeJson, pick: "dark" | "light"): TuiThemeCurrent {
  ...
  const resolved = Object.fromEntries(
    Object.entries(theme.theme)
      .filter(
        ([key]) =>
          key !== "selectedListItemText" && key !== "backgroundMenu" && key !== "thinkingOpacity" && key !== "sessionTitle",
      )
  ...
  return {
    ...
    sessionTitle: theme.theme.sessionTitle === undefined ? resolved.text! : resolveColor(theme.theme.sessionTitle),
...
export async function resolveRunTheme(renderer: CliRenderer): Promise<RunTheme> {
  try {
    ...
    const syntaxTheme: SharedSyntaxTheme = {
      ...
      toolOutput: scrollbackTheme.text,
    }
