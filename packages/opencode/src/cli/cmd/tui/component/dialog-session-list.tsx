...
export function DialogSessionList(props: { initialSessionID?: string } = {}) {
  ...
  const options = createMemo(() => {
    ...
    const grouped: typeof unpinned = []
    const ungrouped: typeof unpinned = []
    ...
    const mapSession = (session: typeof allSessions[number], category: string, showDate: boolean, parseTitle: boolean) => {
      const isDeleting = toDelete() === session.id
      const status = sync.data.session_status?.[session.id]
      const isWorking = status?.type === "busy"
      const parsed = parseTitle ? parseSessionTitle(session.title) : null
      return {
        title: isDeleting ? `Press ${keybind.print("session_delete")} again to confirm` : (parsed?.displayTitle ?? session.title),
    ...
    // Map pinned sessions (bookmarks come first)
    const pinnedOptions = pinned.map((x) => mapSession(x, "Bookmarks:", true, false))

    // Map grouped sessions
    const groupedOptions = grouped.map((session) => {
      const parsed = parseSessionTitle(session.title)
      return mapSession(session, parsed.group ?? "", true, true)
    })
    ...
    const ungroupedOptions = ungrouped.map((session) => {
      ...
      return mapSession(session, category, false, false)
    })
