...
export function DialogSessionList() {
  ...
  const options = createMemo(() => {
    ...
    // Separate ungrouped into grouped and ungrouped
    ...
    const mapSession = (session: typeof allSessions[number], category: string, showDate: boolean, parsedTitle?: { group?: string; displayTitle: string }) => {
      const isDeleting = toDelete() === session.id
      const status = sync.data.session_status?.[session.id]
      const isWorking = status?.type === "busy"
      const title = parsedTitle?.displayTitle ?? session.title
      return {
        title: isDeleting ? `Press ${keybind.print("session_delete")} again to confirm` : title,
    ...
    // Map grouped sessions with their group names as categories
    const groupedOptions = grouped.map((session) => {
      const parsed = parseSessionTitle(session.title)
      return mapSession(session, parsed.group ?? "", true, parsed)
    })

    // Map ungrouped sessions by date
