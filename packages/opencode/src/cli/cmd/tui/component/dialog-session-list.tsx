...
export function DialogSessionList() {
  ...
  const options = createMemo(() => {
    ...
    const mapSession = (session: typeof allSessions[number], category: string, showDate: boolean, displayTitle?: string) => {
      const isDeleting = toDelete() === session.id
      const status = sync.data.session_status?.[session.id]
      const isWorking = status?.type === "busy"
      const parsed = parseSessionTitle(session.title)
      return {
        title: isDeleting ? `Press ${keybind.print("session_delete")} again to confirm` : (displayTitle ?? parsed.displayTitle),
    ...
    const groupedOptions = grouped.map((session) => {
      const parsed = parseSessionTitle(session.title)
      return mapSession(session, parsed.group ?? "", false, parsed.displayTitle)
    })
