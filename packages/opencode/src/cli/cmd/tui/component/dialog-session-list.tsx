...
export function DialogSessionList(props: { initialSessionID?: string } = {}) {
  ...
  const options = createMemo(() => {
    ...
    const grouped = unpinned.filter((x) => parseSessionTitle(x.title).group)
    const plain = unpinned.filter((x) => !parseSessionTitle(x.title).group)
    ...
    const groupedOptions = grouped.map((x) => mapSession(x, parseSessionTitle(x.title).group!, true))

    const plainOptions = plain.map((x) => {
