...
describe("SessionPrompt.substituteArguments", () => {
  test("${1} as only placeholder should swallow remaining args", () => {
  ...
  test("${2} as only placeholder should swallow remaining args", () => {
  ...
  test("${1} should be equivalent to $1", () => {
  ...
  test("${2} should be equivalent to $2", () => {
    const extended = SessionPrompt.substituteArguments("Hello ${2}", ["foo", "bar"])
    const simple = SessionPrompt.substituteArguments("Hello $2", ["foo", "bar"])
    expect(extended.result).toBe(simple.result)
  })
  ...
  test("last placeholder should swallow remaining arguments", () => {
    const result = SessionPrompt.substituteArguments("First: $1, Rest: $2", ["a", "b", "c", "d"])
    expect(result.result).toBe("First: a, Rest: b c d")
  })

  test("last extended placeholder with open end should swallow remaining", () => {
    const result = SessionPrompt.substituteArguments("First: ${1}, Rest: ${2:}", ["a", "b", "c", "d"])
    expect(result.result).toBe("First: a, Rest: b c d")
  })
