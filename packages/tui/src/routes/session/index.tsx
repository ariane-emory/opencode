...
import { BoxRenderable, ScrollBoxRenderable, addDefaultParsers, TextAttributes, RGBA, StyledText, SyntaxStyle } from "@opentui/core"
...
import { renderMarkdownThemedStyled, parseMarkdownSegments } from "../../markdown-renderer"
...
function UserMessage(props: {
  ...
  const segments = createMemo(() => parseMarkdownSegments(text()?.trim() ?? ""))
...
function AssistantMessage(props: { message: AssistantMessage; parts: Part[]; last: boolean }) {
  ...
  createEffect(() => {
    ...
    // Using props.parts does not trigger updates when individual parts change
    ...
    // No running part found - do not show elapsed time
    if (!startTime) {
      setElapsedTime(0)
      return
    }
